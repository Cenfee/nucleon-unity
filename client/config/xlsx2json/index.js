...
function exportJson(args) {
	...
	if (!fs.existsSync(path.join(__dirname, config.xlsx.dest))) {
            fs.mkdirSync(path.join(__dirname, config.xlsx.dest));
        };
		...
		var dest = config.xlsx.dest + path.dirname(element.replace(config.xlsx.srcPath,""));
                xlsx.toJson(path.join(__dirname, element), path.join(__dirname, dest), config.xlsx.head,config.xlsx.arraySeparator);
            });
